...
<body>
    ...
    <script>
        ...
        async function loadDestinations() {
            try {
                const response = await fetch('../Backend/routes/api.php?uri=/api/admin/attractions');
                ...
                updateStats(topDestinations);
                renderTable(topDestinations);
            } catch (error) {
        ...
        function updateStats(destinations) {
            ...
            document.getElementById('cityCount').textContent = destinations.filter(d => d.category === 'city').length;
            document.getElementById('municipalityCount').textContent = destinations.filter(d => d.category === 'municipality').length;
        }
        ...
        function renderTable(destinations) {
            ...
            tbody.innerHTML = destinations.map(dest => `
                <tr>
                    ...
                    <td>${dest.location}</td>
                    <td>
                        <span class="badge ${dest.category === 'city' ? 'badge-success' : 'badge-secondary'}">
                            ${dest.category === 'city' ? 'City' : 'Municipality'}
                        </span>
                    </td>
                    <td>
                        <div class="actions">
        ...
        async function editDestination(id) {
            try {
                ...
                showNotification('Error loading destination', 'error');
            }
        }
